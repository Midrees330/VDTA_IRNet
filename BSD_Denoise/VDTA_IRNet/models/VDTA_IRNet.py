...
class DegradeArea_AwareDecoder(nn.Module):
    ...
    def __init__(self, latent_dims=None):
        ...
        self.shadow_feature_processor = nn.Sequential(  # Input: decoded image (3ch) + Degrade maps (3ch)
    ...
    def forward(self, latent, input_image=None):
        ...
        if input_image is not None:
            ...
            # Combine input and processed output based on shadow mask
...
class VDTA_IRNet(nn.Module):
    ...
    def train_set(self, input, return_shadow=False):
        latent_1 = self.encoder1(input)
