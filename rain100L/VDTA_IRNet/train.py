from utils.data_loader_lol_rain import make_datapath_list, ImageDataset, ImageTransform   
...
class AdvancedDarkObjectPreservationLoss(nn.Module):
    ...
    def detect_multi_threshold_dark_regions(self, image):
        """Multi-threshold dark region detection for robustness"""
    ...
    def compute_advanced_edge_strength(self, image):
        """Advanced edge strength computation using multiple edge filters"""
...
def plot_log(data, save_model_name='model'):
    ...
    # ================ NEW: degrade-SPECIFIC LOSSES PLOT ================
    ax = axes[1, 2]
    ...
    for epoch in range(num_epochs + 1):
        ...
        for n, (images, gt_shadow, gt) in enumerate(progress_bar):
            ...
            try:
                ...
                # In train.py, after computing G_L_non_degrade, add:
                ...
                if epoch >= parser.pretrain_epochs:
                    G_loss = (
                        ...
                        1.0 * G_L_non_shadow +  # ADD THIS LINE - Strong weight for preservation
                    ...
                    G_loss = (
                        ...
                        0.8 * G_L_non_shadow  # ADD THIS LINE - Still important during pretraining
                    )
                ...
                progress_bar.set_postfix({
                    ...
                    'NS': f'{G_L_non_shadow.item():.4f}'  # ADD THIS LINE
                })
        ...
        if enhanced_combined_loss < best_combined_loss:
            ...
            patience_counter = 0
            # Save best model
            torch.save(G1.state_dict(), 'checkpoints/' + save_model_name + 'VDTA_IRNet_Derain_best.pth')
        ...
        if epoch % 10 == 0:
            torch.save(G1.state_dict(), 'checkpoints/' + save_model_name + '_Derain_' + str(epoch) + '.pth')
            if epoch >= parser.pretrain_epochs // 2:
                torch.save(D1.state_dict(), 'checkpoints/' + save_model_name + '_Derain_' + str(epoch) + '.pth')
...
def main(parser):
    ...
    print("Initializing Stable Pure VAE LSR_TowardsGIRNet with Full Latent Propagation...")
    ...
    if parser.load is not None:
        ...
        try:
            G1.load_state_dict(fix_model_state_dict(torch.load('./checkpoints/VDTA_IRNet_Derain_' + parser.load + '.pth')))
            D1.load_state_dict(fix_model_state_dict(torch.load('./checkpoints/VDTA_IRNet_Derain_D_' + parser.load + '.pth')))
    ...
    print("\nTraining completed!")
    print("Best model saved at: checkpoints/VDTA_IRNet_Derain_best.pth")
